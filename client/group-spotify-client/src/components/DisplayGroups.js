    ...
    return (
        <div>
            <div className='mb-5 p-4 card'>
                <div className='is-flex-direction-row' style={{ display: "flex", flexWrap: "wrap", gap: ".5rem" }}>
                    <p className='is-size-3' style={{ flexGrow: "1" }}>Groups you have created </p>
                    <button className='button is-link is-align-self-center' onClick={e => fetchGroups(e.target)}>Refresh <span className='ml-2 is-flex'><FiRefreshCw size={20} /></span></button>
                    <button className='button is-primary is-align-self-center' onClick={e => setShowCreateGroup(true)}>Create New Group</button>
                </div>
                <hr></hr>
                <div className='is-flex is-flex-direction-row is-flex-wrap-wrap is-align-items-start' style={{ "gap": "2em" }}>
                    {showCreateGroup ? <CreateGroupCard show={showCreateGroup} setShow={setShowCreateGroup} fetchGroups={fetchGroups} /> : null}

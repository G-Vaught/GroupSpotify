...
app.post('/addGroup', async (req, res) => {
	...
	try {
		...
		const group = await Group.findById(createdGroup._id).populate(
			'owner users.user'
		);
...
app.post('/getGroupsByUser', async (req, res) => {
	...
	try {
		...
		const groups = await Group.find({
			$or: [{ owner: user._id }, { 'users.user': user._id }],
		})
			.sort({ createdDate: -1 })
			.populate('owner users.user');
...
app.post('/deleteGroup', async (req, res) => {
	...
	if (group.spotifyPlaylistID) {
		await spotifyApi.changePlaylistDetails(group.spotifyPlaylistID, {
			name: group.name + ' - DELETED',
			description: 'Deleted, this playlist will no longer be updated!',
		});
	}
...
async function createPlaylist(group, spotifyApi) {
	const res = await spotifyApi.createPlaylist(group.name, {
		public: false,
